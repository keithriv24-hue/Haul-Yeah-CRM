...
Covers:
  1. Full notification lifecycle for POST /api/tasks/{recordId}/audience:
     ...
     - owner GET → items: []
  2. POST /api/assignments happy path with crew list (Javante as Driver) →
     assignment doc has resolved crew name; javante's GET /api/crew/my-jobs
...
class TestAssignmentsCrewFlow:
    def test_create_assignment_with_crew_and_delete(self, owner_headers, crew_headers,
        ...
        javante = next((u for u in users if u.get("email") == JAVANTE_EMAIL), None)
        assert javante is not None, "javante user not found"
        jav_id = javante.get("id") or javante.get("_id")
        ...
        try:
            ...
            # javante GET /api/crew/my-jobs includes it
            ...
            assert any(j.get("id") == aid or j.get("_id") == aid for j in jobs), \
                f"assignment {aid} not in javante's my-jobs: {jobs}"
        finally:
            # DELETE cleanup

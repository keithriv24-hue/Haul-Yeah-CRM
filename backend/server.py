...
def get_google_creds():
    ...
    if not cid or not secret:
        raise HTTPException(status_code=503, detail="Google Calendar is not set up. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in the secrets panel.")
    return cid, secret
...
def public_base_url() -> str:
    ...
    if not base:
        raise HTTPException(status_code=503, detail="PUBLIC_BASE_URL is not set on the server.")
    return base


def gcal_redirect_uri() -> str:
...
async def gcal_access_token() -> str:
    doc = await mongo_db.settings.find_one({"_id": "google_calendar"})
    if not doc or not doc.get("access_token"):
        raise HTTPException(status_code=400, detail="Google Calendar is not connected yet. Press Connect first.")
    if time.time() > doc.get("expires_at", 0) - 60:
        cid, secret = get_google_creds()
        refresh = doc.get("refresh_token")
        if not refresh:
            raise HTTPException(status_code=400, detail="Google session expired. Press Connect again.")
        async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if resp.status_code != 200:
            raise HTTPException(status_code=400, detail="Google session expired. Press Connect again.")
        t = resp.json()
        doc["access_token"] = t["access_token"]
        await mongo_db.settings.update_one(
            {"_id": "google_calendar"},
            {"$set": {"access_token": t["access_token"], "expires_at": time.time() + t.get("expires_in", 3600)}})
    return doc["access_token"]


oauth_router = APIRouter(prefix="/api/oauth/calendar")


@oauth_router.get("/login")
async def gcal_login(role: str = Depends(require_auth)):
    require_owner(role)
    cid, _ = get_google_creds()
    state = jwt.encode(
        {"type": "gcal_state", "exp": datetime.now(timezone.utc) + timedelta(minutes=10)},
...
async def gcal_callback(code: str = "", state: str = "", error: str = ""):
    base = os.environ.get("PUBLIC_BASE_URL", "").strip().rstrip("/")
    fail = RedirectResponse(f"{base}/projects?gcal=error")
    if error or not code or not state:
        return fail
    try:
        payload = jwt.decode(state, os.environ["JWT_SECRET"], algorithms=[JWT_ALGORITHM])
    ...
    await mongo_db.settings.update_one({"_id": "google_calendar"}, {"$set": update}, upsert=True)
    return RedirectResponse(f"{base}/projects?gcal=connected")


@oauth_router.get("/status")
async def gcal_status(role: str = Depends(require_auth)):
    require_owner(role)
    doc = await mongo_db.settings.find_one({"_id": "google_calendar"})
    return {
        "connected": bool(doc and doc.get("access_token")),
        "email": (doc or {}).get("email"),
        "creds_configured": bool(os.environ.get("GOOGLE_CLIENT_ID", "").strip() and os.environ.get("GOOGLE_CLIENT_SECRET", "").strip()),
    }


@oauth_router.post("/disconnect")
async def gcal_disconnect(role: str = Depends(require_auth)):
    require_owner(role)
    await mongo_db.settings.delete_one({"_id": "google_calendar"})
    return {"ok": True}


@api_router.post("/calendar/sync")
async def gcal_sync(role: str = Depends(require_auth)):
    require_owner(role)
    token = await gcal_access_token()
    records = []
    offset = None
    while True:
        params: Dict[str, Any] = {"pageSize": 100, "returnFieldsByFieldId": "true"}
        if offset:
            params["offset"] = offset
        data = await airtable_request("GET", TABLES["projects"], params=params)
        records.extend(data.get("records", []))
        offset = data.get("offset")
        if not offset:
            break
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for rec in records:
            ...
            job_date = (fields.get(PROJECT_DATE_FIELD) or "")[:10]
            status = fields.get(PROJECT_STATUS_FIELD) or ""
            if not job_date or status == "Cancelled":
                continue
            end_date = (datetime.strptime(job_date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
            body = {
                "summary": fields.get(PROJECT_NAME_FIELD) or "Moving job",
                "description": (
                    f"From: {fields.get(PROJECT_FROM_FIELD) or '—'}\n"
                    f"To: {fields.get(PROJECT_TO_FIELD) or '—'}\n"
                    f"Crew: {fields.get(PROJECT_CREW_FIELD) or '?'} × {fields.get(PROJECT_HOURS_FIELD) or '?'} hrs\n"
                    f"Truck: {fields.get(PROJECT_TRUCK_FIELD) or '—'}\n"
                    f"Status: {status}\nHaul Yeah Moving CRM"
                ),
                "start": {"date": job_date},
                "end": {"date": end_date},
            }
            mapping = await mongo_db.gcal_map.find_one({"_id": rec["id"]})
            if mapping:
                r = await client.patch(f"{GCAL_EVENTS_URL}/{mapping['event_id']}", headers=headers, json=body)
                if r.status_code == 404:
                    r = await client.post(GCAL_EVENTS_URL, headers=headers, json=body)
                    if r.status_code in (200, 201):
                        await mongo_db.gcal_map.update_one({"_id": rec["id"]}, {"$set": {"event_id": r.json()["id"]}})
                        created += 1
                ...
                r = await client.post(GCAL_EVENTS_URL, headers=headers, json=body)
                if r.status_code in (200, 201):
                    await mongo_db.gcal_map.insert_one({"_id": rec["id"], "event_id": r.json()["id"]})
                    created += 1
                elif r.status_code == 403:
                    raise HTTPException(status_code=403, detail="Google says no. Check the Calendar API is enabled in your Google Cloud project.")

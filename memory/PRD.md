...
- **Phase B (NEXT)** — Operations Timeline (auto event feed per job: created, deposit, crew/truck assigned, exec status changes, invoice paid, review) in job detail + 5 moving checklists (Warehouse Departure / Arrival / Loading / Delivery / Completion) attached to existing assignments; checklist completion auto-advances exec_status.
- **Phase C** — Fleet Management: extend trucks w/ maintenance history, mileage, fuel logs, registration/insurance, docs, daily inspection workflow + damage reports w/ photos (reuse object storage), maintenance reminders.

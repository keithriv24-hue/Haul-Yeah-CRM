...
export default function TimeClock() {
  ...
  return (
    <div data-testid="time-clock-page" className="space-y-6 max-w-xl">
      <div>
        <h1 className="text-2xl font-bold text-[#1B2A4A]">Time Clock</h1>
        <p className="text-sm text-slate-500">Punch in when you start, punch out when you're done.</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6 text-center">
        {data === null ? (
          ...
          <>
            <p className="text-xs uppercase tracking-wide text-emerald-600 font-bold">On the clock</p>
            <p data-testid="clock-elapsed" className="text-4xl font-bold text-[#1B2A4A] tabular-nums mt-1">
            ...
            <Button data-testid="clock-out-btn" disabled={busy} onClick={() => punch("out")} className="w-full mt-4 h-14 text-lg gap-2 bg-red-600 hover:bg-red-700">
              {busy ? <Loader2 className="w-5 h-5 animate-spin" /> : <LogOut className="w-5 h-5" />} Clock out
            </Button>
          </>
        ) : (
          <>
            <AlarmClock className="w-10 h-10 text-slate-300 mx-auto" />
            <p className="text-sm text-slate-500 mt-2">You're off the clock.</p>
            <Button data-testid="clock-in-btn" disabled={busy} onClick={startClockIn} className="w-full mt-4 h-14 text-lg gap-2 bg-emerald-600 hover:bg-emerald-700">
      ...
      {weeks.length > 0 && (
        <div data-testid="weekly-totals" className="grid grid-cols-2 gap-3">
          {weeks.map(([wk, hrs]) => (
            <div key={wk} className="bg-white rounded-lg border border-slate-200 p-4">
              <p className="text-[11px] uppercase tracking-wide text-slate-400">Week {wk.split("-W")[1]}</p>
              <p className="text-xl font-bold text-[#1B2A4A]">{hrs} hrs</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div>
        <h2 className="text-sm font-bold uppercase tracking-wide text-slate-500 mb-2">Recent punches</h2>
        <div className="bg-white rounded-lg border border-slate-200 divide-y divide-slate-100">
          {(data?.entries || []).slice(0, 14).map((e) => (
            <div key={e.id} data-testid="time-entry-row" className="flex items-center justify-between px-4 py-2.5 text-sm">
              <div>
                <p className="font-medium text-[#1B2A4A]">{fmtDate(e.clock_in.at.slice(0, 10))}</p>
                <p className="text-xs text-slate-500">
                  {fmtTime(e.clock_in.at)} → {e.clock_out ? fmtTime(e.clock_out.at) : "still on"}
                  {e.job_name ? ` · ${e.job_name}` : ""}
                </p>
              </div>
              <span className="font-bold text-[#1B2A4A]">{e.hours != null ? `${e.hours}h` : "—"}</span>
            </div>
          ))}
          {data && data.entries.length === 0 && <p className="text-sm text-slate-400 px-4 py-6 text-center">No punches yet.</p>}
        </div>
      </div>
        ...
        <AlertDialogContent data-testid="gps-consent-dialog">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display flex items-center gap-2">
              <MapPin className="w-5 h-5 text-[#E8743B]" /> Location while you work
            </AlertDialogTitle>

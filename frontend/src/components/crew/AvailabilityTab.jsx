    ...
    <Dialog open={!!day} onOpenChange={() => onOpenChange(null)}>
      <DialogContent data-testid="availability-day-dialog">
        ...
        <div className="space-y-2">
          {crew.map((u) => (
            <div key={u.user_id} className="flex items-center justify-between text-sm rounded-md border border-slate-100 px-3 py-2">
              <span className="font-semibold text-[#1B2A4A]">{u.name}</span>
              <div className="flex items-center gap-2">
                <span className={`text-xs font-semibold ${offIds.has(u.user_id) ? "text-red-600" : "text-emerald-600"}`}>
  ...
  return (
    <div className="mt-4 space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="font-display font-bold text-[#1B2A4A] w-40 text-center" data-testid="availability-month-label">
        ...
        <div className="flex items-center gap-3 ml-auto text-[11px] text-slate-500">
          <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded bg-orange-100 border border-orange-200 inline-block" /> weekend</span>
          <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded ring-2 ring-red-400 inline-block" /> short on crew</span>
        </div>
      </div>

      {shortWeekends.length > 0 && (
        <div data-testid="short-weekend-banner" className="border border-red-300 bg-red-50 rounded-lg px-4 py-3 text-sm text-red-800 flex items-start gap-2">
      ...
      {data === null ? (
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-3" data-testid="availability-grid">
          <div className="grid grid-cols-7 gap-1 mb-1">
            {WEEKDAYS.map((w, i) => (
              <div key={w} className={`text-center text-[11px] font-bold uppercase ${i === 0 || i === 6 ? "text-[#E8743B]" : "text-slate-400"}`}>{w}</div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-1">
            ...
            {days.map((d) => (
              <button
                ...
                className={`text-left rounded-md border p-1.5 min-h-[84px] transition-colors hover:border-[#E8743B]/60 ${
                  d.weekend ? "bg-orange-50/80 border-orange-200" : "bg-white border-slate-200"
                } ${d.short ? "ring-2 ring-red-400" : ""}`}
              >
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-bold ${d.date === today ? "text-[#E8743B]" : "text-[#1B2A4A]"}`}>{Number(d.date.slice(8))}</span>
                  {d.short && <AlertTriangle className="w-3.5 h-3.5 text-red-500" />}
                </div>
                {d.jobs > 0 && (
                  <span className="inline-block mt-0.5 text-[10px] font-bold bg-[#1B2A4A] text-white rounded px-1">
                    {d.jobs} job{d.jobs === 1 ? "" : "s"} · needs {d.needed}
                  </span>
                )}
                <div className={`text-[10px] font-semibold mt-0.5 ${d.short ? "text-red-600" : "text-emerald-600"}`}>
                  {d.available.length} available
                </div>
                {d.off.slice(0, 3).map((o) => (
                  <div key={o.user_id} className="text-[10px] text-red-600 truncate">✕ {o.name}</div>
                ))}
                {d.off.length > 3 && <div className="text-[10px] text-red-400">+{d.off.length - 3} more off</div>}
              </button>
            ))}
          </div>
        </div>
